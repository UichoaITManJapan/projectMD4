...
<table border="1">
    ...
    <tr>
        ...
        <th>Tên đơn hàng</th>
        <th>Tên sản phẩm</th>
        <th>Giá sản phẩm lúc mua(yên)</th>
        <th>Số lượng sản phẩm lúc mua</th>
        <th>Thành tiền(giá x số lượng)(yên)</th>
    </tr>
    </thead>
    <tbody>
    <c:set var="stt" value="0" scope="page"/>
    <c:forEach items="${billDetailList}" var="billDel">
        <c:set var="stt" value="${stt+1}" scope="page"/>
        <tr>
            <td>${stt}</td>
            <td>${billDel.billDelId}</td>
            <td>${billDel.bill.billName}</td>
            <td>${billDel.product.productName}</td>
            <td>${billDel.product.price}</td>
            <td>${billDel.quantity}</td>
            <td>${(billDel.product.price)*(billDel.quantity)}</td>
        </tr>
    </c:forEach>
    </tbody>
</table>
